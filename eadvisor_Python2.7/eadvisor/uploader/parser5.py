...
def check_CoreUnits(text):
	...
	return result
##########################################################################
def check_GradUnits(text,unit=0):
	global GRAD_raw
	# Interprets status flags for GE section
	result = parse_checkReqs(text)
	if (result == 2):
...
def update_classList():
	...
	GE_list.append(GRAD_list.pop(0))
##########################################################################
def main(FILENAME='TDA.txt',TRACK=3):
	global GE_list
	global TAKE_list
	global IP_list
	global GRAD_list
	...
	if (MAJOR.find('BS  CPSC') == -1):
		...
		check_GradUnits('GENERAL EDUCATION RESIDENCE',2)
		check_GradUnits('GENERAL EDUCATION UPPER DIVISION',2)
		check_GradUnits('GENERAL EDUCATION UNITS',2)
		...
		check_GradUnits('G.P.A. OF ALL COURSES',3)
		if (check_GradUnits('UNIT REQUIREMENTS',0) == -2):
			check_GradUnits('UNITS EARNED AT CSUF',2)
			check_GradUnits('CSUF GRADE POINT AVERAGE',1)
			check_GradUnits('UPPER DIVISION UNITS EARNED',1)
			check_GradUnits('RESIDENCE UPPER DIVISION UNITS',1)
			check_GradUnits('MAJOR Residence UPPER DIVISION',1)
		check_GradUnits('TRANSFER INSTITUTION TOTALS',1)
		check_GradUnits('CUMULATIVE NUMBER OF UNITS',2)
		check_GradUnits('CUMULATIVE GRADE POINT AVERAGE',2)
